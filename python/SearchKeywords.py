...
def fetch_mobile_index():
    ...
    if r.status_code == 200:
        response = json.loads(r.json()['data'])
        for keyword in response['itemsInfo']['items']:
            rst.append(keyword[u'search_term'])
    return rst
...
def fetch():
    ...
    for keyword in keywords:
        if not Database.Redis.zscore(RedisKey, keyword):
            Database.Redis.zadd(RedisKey, keyword, time.time())
    ...
    if r.status_code == 200:
        ...
        for kw in response['trendsByDateList'][0]['trendsList']:
            if not Database.Redis.zscore(RedisKey, kw['title']):
                Database.Redis.zadd(RedisKey, kw['title'], time.time())
